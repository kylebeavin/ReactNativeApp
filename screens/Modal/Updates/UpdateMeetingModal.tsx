...
const UpdateMeetingModal: React.FC<Props> = ({navigation, meeting}) => {
    //#region === Use State Variables ===//
    const [name, setName] = useState(meeting.title);
    const [date, setDate] = useState(getDateStringsFromDate(meeting.meeting_time).date);
    const [startTime, setStartTime] = useState(getDateStringsFromDate(meeting.meeting_time).time);
    const [endTime, setEndTime] = useState("");
    const [street, setStreet] = useState(meeting.address_street);
    const [city, setCity] = useState(meeting.address_city);
    const [state, setState] = useState(meeting.address_state);
    const [zip, setZip] = useState(meeting.address_zip);
    const [account, setAccount] = useState(meeting.account_id);
    const [accountList, setAccountList] = useState<Account[]>();
    const [contact, setContact] = useState(meeting.contact_id);
    ...
    const updateMeeting = async () => {
      ...
      const data = await fetch(
        `${Configs.TCMC_URI}/api/meetings/${updatedMeeting._id}`,
        {
          method: "PUT",
    ...
    return (
      <View>
        ...
        <ModalButtons navigation={navigation} save={() => updateMeeting()} />
