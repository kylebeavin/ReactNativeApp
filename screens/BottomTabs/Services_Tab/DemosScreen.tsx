...
const DemosScreen: React.FC<Props> = ({navigation}) => {
  ...
  const getDemos = async () => {
    ...
    await fetch(`${Configs.TCMC_URI}/api/demosBy`, {
  ...
  return (
    <View style={styles.screen}>
      ...
      {demos.length === 0 ? null : (
        <AppAddNew title="DEMO" modal="CreateDemoModal" />
      )}
      ...
      {isLoading ? (
        <ActivityIndicator color={Colors.SMT_Primary_2} animating={true} />
          ...
          <View>
            ...
            {/* Orders List */}
            {demos.length === 0 ? (
              <AppEmptyCard entity="demos" modal="CreateDemoModal" />
            ) : (
              demos.map((u, i) => {
                return (
                  <View style={styles.card} key={i}>
                    ...
                    <View style={styles.column2}>
                      <AppButton
                        ...
                        onPress={() => navigation.navigate("Modal", {modal: "UpdateDemoModal", item: u})}
                      />
                    </View>
                  </View>
                );
