...
const AppCardDrawer: React.FC<Props> = ({
  navigation,
  isVisible,
  contacts,
  account,
  ...
  const openMailTo = (email: string) => {
    Linking.canOpenURL('mailto:').then((data) =>
      data ? Linking.openURL(`mailto:${email}`) : null,
    );
  };

  const openPhoneTo = (phone: string) => {
    Linking.canOpenURL('tel:').then((data) =>
      data ? Linking.openURL(`tel:${phone}`) : null,
    );
  };
          ...
          <View style={styles.buttonContainer}>
            ...
            <View style={styles.contactButton}>
              <AppButton
                ...
                onPress={() => openPhoneTo(item.phone)}
                backgroundColor={Colors.SMT_Secondary_2}
              />
            </View>
          </View>
